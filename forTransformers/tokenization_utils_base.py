...
class PreTrainedTokenizerBase(SpecialTokensMixin, PushToHubMixin):
    ...
    def apply_chat_template(
        ...
        **kwargs,
        ...
        if tokenize:
            single_prompt = rendered_chat[0] if is_batched else rendered_chat
            out = self(
                rendered_chat,
                padding=padding,
                truncation=truncation,
                max_length=max_length,
                add_special_tokens=not (self.bos_token is not None and single_prompt.startswith(self.bos_token)),
